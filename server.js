...
function meta(f, attack = f.attack, aim = f.attackAim) {
  ...
  if (attack === "counter") {
    ...
    if (p === "knight") Object.assign(m, { dmg: 13, kb: 36, lift: -20, cd: 100, wall: 42 });
  ...
  if (p === "king") {
    if (attack === "light") Object.assign(m, { duration: 25, activeA: 9, activeB: 14, dmg: 8, kb: 24, lift: -4, cd: 22, wall: 30 });
    if (attack === "airLight") Object.assign(m, { duration: 26, activeA: 8, activeB: 15, dmg: 8, kb: 24, lift: 12, cd: 23, wall: 28 });
    if (attack === "crouchLight") Object.assign(m, { duration: 24, activeA: 9, activeB: 14, dmg: 7, kb: 20, lift: -20, cd: 22, wall: 24 });
    if (attack === "heavy") Object.assign(m, { duration: 54, activeA: 23, activeB: 31, dmg: 20, kb: 56, stamina: 18, cd: 86, armor: 32, breakArmor: true, wall: 78 });
    if (attack === "crouchHeavy") Object.assign(m, { duration: 48, activeA: 18, activeB: 27, dmg: 17, kb: 42, lift: -26, stamina: 15, cd: 76, armor: 22, wall: 62 });
    if (attack === "airHeavy") Object.assign(m, { duration: 50, activeA: 17, activeB: 29, dmg: 18, kb: 38, lift: 34, stamina: 16, cd: 78, breakArmor: true, wall: 56 });

    if (attack === "special" || attack === "airSpecial") {
      if (aim === "down") Object.assign(m, { duration: 60, activeA: 22, activeB: 31, dmg: 18, kb: 18, lift: -9, stamina: 26, cd: 210, armor: 34, grab: true, throwPower: 80, wall: 74 });
      else if (aim === "up") Object.assign(m, { duration: 58, activeA: 20, activeB: 34, dmg: 17, kb: 26, lift: -56, stamina: 24, cd: 190, armor: 24, breakArmor: true, wall: 42 });
      else Object.assign(m, { duration: 68, activeA: 27, activeB: 39, dmg: 21, kb: 64, lift: -10, stamina: 30, cd: 230, armor: 38, breakArmor: true, wall: 92 });
    }
  }
  ...
  if (p === "knight") {
    if (attack === "light") Object.assign(m, { duration: 19, activeA: 5, activeB: 12, dmg: 8, kb: 20, lift: -6, cd: 14, wall: 24 });
    if (attack === "airLight") Object.assign(m, { duration: 20, activeA: 4, activeB: 13, dmg: 8, kb: 22, lift: 8, cd: 15, wall: 28 });
    if (attack === "crouchLight") Object.assign(m, { duration: 18, activeA: 5, activeB: 12, dmg: 7, kb: 18, lift: -18, cd: 15, wall: 22 });
    if (attack === "heavy") Object.assign(m, { duration: 34, activeA: 9, activeB: 22, dmg: 17, kb: 38, lift: -14, stamina: 11, cd: 48, armor: 10, breakArmor: true, wall: 55 });
    if (attack === "crouchHeavy") Object.assign(m, { duration: 32, activeA: 8, activeB: 20, dmg: 15, kb: 34, lift: -26, stamina: 10, cd: 44, armor: 8, breakArmor: true, wall: 48 });
    if (attack === "airHeavy") Object.assign(m, { duration: 36, activeA: 7, activeB: 25, dmg: 16, kb: 33, lift: 22, stamina: 10, cd: 46, breakArmor: true, wall: 50 });
    if (attack === "airSpecial") {
      Object.assign(m, {
        duration: 34,
        activeA: 1,
      ...
      if (aim === "up") Object.assign(m, { duration: 42, activeA: 8, activeB: 24, dmg: 14, kb: 25, lift: -52, stamina: 16, cd: 96, armor: 12, breakArmor: true, wall: 36 });
      else if (aim === "down") Object.assign(m, { duration: 42, activeA: 9, activeB: 24, dmg: 16, kb: 42, lift: 16, stamina: 17, cd: 105, armor: 18, breakArmor: true, wall: 60 });
      else Object.assign(m, { duration: 40, activeA: 8, activeB: 25, dmg: 15, kb: 38, lift: -10, stamina: 16, cd: 100, armor: 14, breakArmor: true, wall: 56 });
    }
  }
  ...
  if (p === "queen") {
    if (attack === "light") Object.assign(m, { duration: 16, activeA: 4, activeB: 11, dmg: 14, kb: 19, cd: 12 });
    if (attack === "heavy") Object.assign(m, { duration: 30, activeA: 8, activeB: 17, dmg: 20, kb: 34, stamina: 10, cd: 40, breakArmor: true });
    if (attack === "crouchHeavy") Object.assign(m, { duration: 30, activeA: 8, activeB: 17, dmg: 19, kb: 30, lift: -20, stamina: 10, cd: 40 });
    if (attack === "airHeavy") Object.assign(m, { duration: 30, activeA: 8, activeB: 16, dmg: 19, kb: 28, lift: 17, stamina: 10, cd: 40 });
    if (attack === "special" || attack === "airSpecial") {
      if (aim === "up") Object.assign(m, { duration: 52, activeA: 12, activeB: 27, dmg: 23, kb: 24, lift: -43, stamina: 22, cd: 172, breakArmor: true });
      else if (aim === "down") Object.assign(m, { duration: 52, activeA: 12, activeB: 27, dmg: 23, kb: 38, lift: 15, stamina: 22, cd: 172, breakArmor: true });
      else Object.assign(m, { duration: 54, activeA: 12, activeB: 29, dmg: 24, kb: 42, stamina: 22, cd: 176, breakArmor: true });
    }
  }
...
function box(f) {
  ...
  if (p === "knight") {
    if (a === "light") return forward(150, 70, 20);
    if (a === "airLight") return body(160, 110, -4, 42);
    if (a === "crouchLight") return forward(140, 46, f.h * 0.62);
    if (a === "heavy") return body(230, 126, -18, 72);
    if (a === "crouchHeavy") return forward(200, 74, f.h * 0.46);
    if (a === "airHeavy") return body(210, 160, -16, 40);
    if (a === "airSpecial") return { x: f.x - 54, y: f.y - 54, w: f.w + 108, h: f.h + 108 };
    if (a === "special") {
      if (aim === "up") return up(155, 220, 14);
      if (aim === "down") return body(210, 130, f.h * 0.25, 62);
      return body(220, 125, -6, 80);
    }
  }
...
function updateQueenUltimate(f, enemy, game, input, u) {
  ...
  if (u.phase === "hyper") {
    if (f.lightCd > 0) f.lightCd = Math.max(0, f.lightCd - 2);
    if (f.heavyCd > 0) f.heavyCd = Math.max(0, f.heavyCd - 2);
    if (f.specialCd > 0) f.specialCd = Math.max(0, f.specialCd - 2);
    ...
    if (u.timer < 30 && enemy?.hp > 0) {
      const cx = f.x + f.w / 2;
      const cy = f.y + f.h / 2;
      const ex = enemy.x + enemy.w / 2;
      const ey = enemy.y + enemy.h / 2;

      enemy.vx += clamp((cx - ex) * 0.06, -7.2, 7.2);
      enemy.vy += clamp((cy - ey) * 0.045, -5.4, 5.4);
      enemy.stun = Math.max(enemy.stun, 6);
      enemy.hurt = Math.max(enemy.hurt, 6);
    }

    if (!u.finisherDone && u.timer === 30) {
      u.finisherDone = true;
      const damage = clamp(42 + u.hits * 4.2, 42, 98);

      const hb = makeHitbox(f, f.x + f.w / 2 - 430, f.y - 155, 860, 400, {
        dmg: damage,
        kb: 84,
        lift: -44,
        wall: 128,
...
function startAttack(f, base, input, game) {
  ...
  if (base === "counter") {
    ...
    if (p === "knight") {
      if (move === "airSpecial") {
        f.script = knightAirPattern(f, input);
      } else if (aim === "up") {
        f.vy = -13.8;
        f.vx += d * 5.5;
        f.armor = Math.max(f.armor, 12);
      } else if (aim === "down") {
        f.vx += d * 10;
        f.vy = Math.max(f.vy, 7);
        f.armor = Math.max(f.armor, 18);
      } else {
        f.script = fixed(d * 18, -1.5, 10);
